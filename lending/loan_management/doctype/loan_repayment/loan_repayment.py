...
def calculate_amounts(
	...
	try:
		...
		query = (
			frappe.qb
			.from_(LoanSecurityDeposit)
			.select(Sum(LoanSecurityDeposit.available_amount))
			.where(LoanSecurityDeposit.loan == against_loan)
		)

		result = query.run()
		amounts["available_security_deposit"] = result[0][0] or 0
...
def get_last_demand_date(
    ...
    query = (
        frappe.qb
        .from_(LoanDemand)
        .select(Max(LoanDemand.demand_date))
        .where((LoanDemand.loan == loan) &
    ...
    # ✅ Check if status column exists before adding to query
    loan_demand_fields = [f.fieldname for f in frappe.get_meta("Loan Demand").fields]
    if status and "status" in loan_demand_fields:
        query = query.where(LoanDemand.status == status)

    result = query.run()
    last_demand_date = result[0][0] if result and result[0][0] else None

    # fallback to last disbursement date
    if not last_demand_date:
        last_demand_date = get_last_disbursement_date(
            loan, posting_date, loan_disbursement=loan_disbursement
        )
...
def get_latest_accrual_date(loan, posting_date, interest_type="Interest", loan_disbursement=None):
	...
	query = (
		frappe.qb
		.from_(LoanInterestAccrual)
		.select(Max(LoanInterestAccrual.posting_date))
		.where(
	...
	result = query.run()
	latest_accrual_date = result[0][0] if result and result[0][0] else None
...
def get_accrued_interest(
	...
	query = (
		frappe.qb
		.from_(LoanInterestAccrual)
		.select(Sum(LoanInterestAccrual.interest_amount))
		.where(
			(LoanInterestAccrual.loan == loan) &
			(LoanInterestAccrual.docstatus == 1) &
			(LoanInterestAccrual.posting_date < posting_date) &
	...
	result = query.run()
	accrued_interest = result[0][0] if result and result[0][0] else 0

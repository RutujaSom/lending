...
def get_last_accrual_date(
	...
	last_interest_accrual_date = result[0][0] if result and result[0][0] else None
...
from pypika.functions import Max, Min
...
def get_last_disbursement_date(loan, posting_date, loan_disbursement=None):
    ...
    schedule_type = frappe.db.get_value("Loan", loan, "repayment_schedule_type", cache=True)
    agg_func = Min if schedule_type == "Line of Credit" else Max

    # Start building query
    query = (
        frappe.qb
        .from_(LoanDisbursement)
        .select(agg_func(LoanDisbursement.disbursement_date))
